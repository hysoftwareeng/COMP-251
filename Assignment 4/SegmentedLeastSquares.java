...
public class SegmentedLeastSquares {
	...
	public void computeOptIterative( ){
		//   ADD YOUR CODE HERE						  
		opt[0] = costSegment; //base cases
		opt[1] = opt [0];
		int j = 2; //next segment
		while (j < opt.length){
			opt[j] = e_ij[0][j] + costSegment;;	//value used to compare
			double newOpt = 0.0; //initialize potential new Opt to 0.0
			int i = 1;
			for (i=1; i<=j; i++){
				newOpt = opt[i-1]+e_ij[i][j]+ costSegment;
				opt[j] = Math.min(opt[j], newOpt);	//choose the minimum of the two
			}
		j++;
		}
	}
	...
	public double computeOptRecursive(int j){
		...
		opt[0] = costSegment; //base cases
		opt[1] = opt [0];
		if (opt[j] != 0){	//opt[j] already calculated
			return opt[j];
		}
		else {
			...
			int i;
			for (i=1; i<=j; i++){
				newOpt = computeOptRecursive(i-1)+e_ij[i][j]+ costSegment;
				opt[j] = Math.min(opt[j], newOpt);	//choose the minimum of the two
			}
			return opt[j];
		}
	...
	public void computeSegmentation(int j){
		...
		if (j>0){
			...
			int newPointIndex = 0;
			for (i=0; i<=j; i++){
				if (i==0){ //checking the conditions given in the pseudocode
					if (opt[j] == e_ij[i][j] + costSegment){ //base case, no opt[i-1]
						newPointIndex = i;
						...
						newSegment.a = a[i][j];
						newSegment.b = b[i][j];
						newSegment.error = e_ij[i][j];
						lineSegments.add(newSegment);	//add the segment
						computeSegmentation(newPointIndex-1); 
						break;
					}
				}
				else if (opt[j] == opt[i-1] + e_ij[i][j] + costSegment){ //check to see condition is true
					newPointIndex = i;
					...
					newSegment1.a = a[i][j];
					newSegment1.b = b[i][j];
					newSegment1.error = e_ij[i][j];
					lineSegments.add(newSegment1);	//add the segment
					computeSegmentation(newPointIndex-1);
				}
			}
		}
